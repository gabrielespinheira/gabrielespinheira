...
import { MacDock, MacDockItem } from "@/components/ui/dock";
	...
	return (
		...
		>
			...
			<motion.div
				...
				className="flex h-12 w-12 cursor-default items-center justify-center rounded-xl border border-white/10 shadow-sm md:h-18 md:w-18"
...
export default function Hero() {
	...
	return (
		<main className="relative mx-auto flex min-h-dvh max-w-5xl flex-col items-center justify-center overflow-hidden bg-background px-4 py-12 pb-32 md:px-8 md:py-0 md:pb-40">
			...
			>
				...
				<motion.div variants={item} className={cn(cardClass, "col-span-2")}>
					...
					<div className="flex flex-col gap-5 p-6 md:p-8">
						...
						<div className="flex flex-col gap-2.5 font-light text-muted-foreground text-sm leading-relaxed">
							<p>
			...
			{/* OS Dock Section */}
			<div className="pointer-events-none fixed bottom-2 left-1/2 z-50 flex w-full max-w-[100vw] -translate-x-1/2 justify-center md:bottom-4">
				<motion.div
					variants={item}
					className="no-scrollbar pointer-events-auto flex w-full max-w-[95vw] justify-start overflow-x-auto px-4 pt-14 pb-2 md:justify-center"
				>
					<MacDock className="w-max max-w-none shrink-0">
						{apps.map((app) => (
							<MacDockItem key={app.label} label={app.label} icon={app.icon} />
						))}
					</MacDock>
				</motion.div>
			</div>
